...
esp_err_t firmware_ota_ble_write(uint32_t offset, const uint8_t *data, size_t length, uint8_t *progress_percent) {
    ...
    if (progress_percent) {
        *progress_percent = (uint8_t)percent;
    }
...
esp_err_t firmware_ota_ble_finish(char *received_version, size_t received_version_size) {
    ...
    if (received_version && received_version_size > 0) {
        strlcpy(received_version, incoming.version, received_version_size);
    }
...
esp_err_t firmware_ota_update_https(const char *url, const char *bearer_token, const char *ota_access_key,
    ...
    int last_reported_bytes = 0;
    if (image_size > 0)
        ESP_LOGI(FW_TAG, "Tamanho do firmware OTA: %d bytes", image_size);

    do {
        err = esp_https_ota_perform(handle);
        int downloaded = esp_https_ota_get_image_len_read(handle);

        if (image_size > 0 && downloaded >= 0) {
            int percent = (int)(((int64_t)downloaded * 100) / image_size);
            if (percent > 100)
                percent = 100;
            int milestone = (percent / 10) * 10;
            if (milestone >= last_reported_percent + 10) {
                last_reported_percent = milestone;
                ESP_LOGI(FW_TAG, "Download OTA: %d%% (%d/%d bytes)", percent, downloaded, image_size);
            }
        } else if (downloaded >= last_reported_bytes + (128 * 1024)) {
            last_reported_bytes = downloaded;
            ESP_LOGI(FW_TAG, "Download OTA: %d bytes recebidos", downloaded);
        }
    ...
    if (image_size > 0 && last_reported_percent < 100)
        ESP_LOGI(FW_TAG, "Download OTA: 100%% (%d/%d bytes)", image_size, image_size);
